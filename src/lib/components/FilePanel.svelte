<script lang="ts">
import { ChevronLeft, Circle, File, FileCode, Folder, RefreshCw, Save, X } from "@lucide/svelte";
...
<aside class="file-panel" aria-label="Project files">
	<header class="panel-header">
		...
		<div>
			<Button variant="ghost" size="icon-sm" aria-label="Refresh files" onclick={() => loadDirectory(directory)}
				><RefreshCw size={15} /></Button
			><Button variant="ghost" size="icon-sm" aria-label="Close files" onclick={close}><X size={16} /></Button>
		</div>
	</header>
	<div class="file-browser">
		<div class="directory-heading">
			<Button
				...
				><ChevronLeft /></Button
			><span class="truncate" title={directory}>{directory || project.name}</span>
		</div>
		<div class="file-list">
	...
	{#if error}
		<div class="file-error" role="alert">{error}</div>
	{/if}
	{#if file}
		<div class="editor-toolbar">
			...
			><Button size="sm" disabled={!dirty || saving || loading} onclick={save}
				><Save />{saving ? "Saving" : "Save"}</Button
			>
		</div>
		<textarea
			class="code-editor"
