<script lang="ts">
	...
	const ready = $derived(
		app.connected && !!app.project && (!app.requiresAuth || !!app.account),
	);
	onMount(() => {
	...
	async function send() {
		...
		prompt = "";
		followBottom = true;
		if (!(await app.send(text))) prompt = text;
		await tick();
		composer?.focus();
	}
	...
	async function canLeaveFiles() {
		return (
			!fileDirty ||
			(await confirm(
				"Discard unsaved file changes before switching projects?",
	...
	async function removeProject() {
		...
		if (
			await confirm(
				"Remove this project from ReCodex? Files and Codex conversations will stay on disk.",
		...
		<aside class="sidebar">
			<div class="brand-row">
				<a
					...
					><span class="brand-mark"
						><Code2 size={21} strokeWidth={2.1} /></span
					>ReCodex</a
				><Button
					variant="ghost"
					size="icon"
					aria-label="Hide sidebar"
					onclick={() => (sidebar = false)}><PanelLeftClose /></Button
				>
			</div>
			<nav class="primary-nav" aria-label="Workspace">
				<button onclick={() => app.newChat()} disabled={app.busy}
					><SquarePen size={18} /><span>New chat</span><kbd
						>Ctrl ⇧ O</kbd
					></button
				>
				<button onclick={() => (searchOpen = !searchOpen)}
					><Search size={18} /><span>Search chats</span><kbd
						>Ctrl K</kbd
					></button
				>
			</nav>
			{#if searchOpen}<div class="sidebar-search">
					<Input
			...
			<div class="section-heading">
				<span>Projects</span><Button
					...
					onclick={addProject}><Plus /></Button
				>
			</div>
			<div class="project-list">
				{#each app.projects as project}<button
						class:selected={app.project?.path === project.path}
						disabled={app.busy}
						onclick={() => chooseProject(project)}
						title={project.path}
						><Folder size={17} /><span class="truncate"
							>{project.name}</span
						>{#if app.project?.path === project.path}<ChevronDown
								size={13}
							/>{/if}</button
					>{/each}
				<button
					...
					disabled={app.busy || !isTauri()}
					onclick={addProject}
					><FolderPlus size={17} /><span>Open a project</span></button
				>
			</div>
			<div class="section-heading recent-heading">
				<span>Recent chats</span>{#if app.loading}<LoaderCircle
						class="spin"
						size={13}
					/>{/if}
			</div>
			<div class="thread-list">
				{#each filteredThreads as thread}<button
						...
						></button
					>{/each}
				{#if !filteredThreads.length}<div class="sidebar-empty">
						{search
							? "No matching chats."
							: "Your conversations will appear here."}
					</div>{/if}
				{#if app.cursor}<Button
						variant="ghost"
						size="sm"
						disabled={app.busy}
			...
			<div class="sidebar-footer">
				<button
					...
					><span class="avatar"
						>{app.account?.email?.[0]?.toUpperCase() || "R"}</span
					><span
						><strong
		...
		<header class="topbar">
			<div class="topbar-left">
				...
				<Dropdown.Root
					><Dropdown.Trigger
						class="model-trigger"
						disabled={!app.models.length || app.busy}
					...
					><Dropdown.Content align="start" class="w-80"
						...
						>{#each app.models as model}<Dropdown.Item
								...
								{#if app.model === model.model}<Check
										size={15}
									/>{/if}</Dropdown.Item
			...
			<div class="topbar-right">
				{#if app.project}<span class="project-breadcrumb"
						><Folder size={14} />{app.project.name}</span
					><Button
						variant="ghost"
						size="sm"
					...
					><Dropdown.Root
						...
						><Dropdown.Content align="end"
							...
							><Dropdown.Item onclick={removeProject}
								>Remove project</Dropdown.Item
							></Dropdown.Content
						></Dropdown.Root
					>{:else}<span class="topbar-hint"
						>Your ideas, in progress.</span
					>{/if}
			</div>
		</header>
		<div class="workspace-body">
			<section class="chat-pane" aria-label="Chat">
				{#if !isTauri()}<div class="preview-banner">
						Browser preview · Open the desktop app to connect to
						Codex and your files.
					</div>{/if}
				{#if app.error}<div class="error-banner" role="alert">
						<span>{app.error}</span><Button
							variant="ghost"
							size="icon-xs"
							aria-label="Dismiss error"
							onclick={() => (app.error = "")}><X /></Button
						>
					</div>{/if}
				<div
					class="chat-scroll"
				...
				>
					{#if app.items.length === 0}
						<div class="welcome">
							<div class="welcome-eyebrow">
								<span></span>A little focus. A lot of
								possibility.
							</div>
							<h1>What will you build?</h1>
							<p>
								Bring an idea. Make it real.<br />Your code and
								conversations, in one quiet workspace.
							</p>
							{#if !app.project}<Button
									variant="outline"
									size="lg"
									onclick={addProject}
									disabled={!isTauri() || app.busy}
									><FolderPlus size={16} />Open a project<ArrowUpRight
										size={14}
									/></Button
								>{:else}<div class="workspace-pill">
									<Folder size={14} /><span
					...
					{:else}
						<div class="messages">
							{#each app.items as item (item.id)}<Message
									{item}
								/>{/each}{#if app.activeTurn || app.sending}<div
									class="working"
								>
									<span class="working-dot"></span>{app
										.approvals.length
										? "Waiting for your input"
										: "Working on it…"}
								</div>{/if}{#each app.approvals as event (event.id)}<Approval
									{event}
				...
				<div class="composer-area">
					...
					>
						...
						<div class="composer-toolbar">
							<div class="composer-context">
								...
								>{#if app.selectedModel?.supportedReasoningEfforts.length}<span
										class="toolbar-divider"
									></span><select
										aria-label="Reasoning effort"
										disabled={app.busy}
										bind:value={app.effort}
										>{#each app.selectedModel.supportedReasoningEfforts as option}<option
					...
					<div class="composer-footnote">
						<span
							><ShieldCheck size={12} />Review changes before you
							commit</span
						><span>Shift + Enter for a new line</span>
					</div>
					{#if isTauri() && !app.connected}<div
						...
						>
							...
							>
						</div>{:else if app.connected && app.requiresAuth && !app.account}<div
							class="connection-action"
						>
							<Button size="sm" onclick={() => app.login()}
								>Sign in with ChatGPT</Button
							>
						</div>{/if}
				</div>
			</section>
			{#if filesOpen && app.project}{#key app.project.path}<FilePanel
						project={app.project}
	...
	><Dialog.Content class="settings-dialog"
		><Dialog.Header
			><Dialog.Title>Settings</Dialog.Title><Dialog.Description
				>A workspace that feels like yours.</Dialog.Description
			></Dialog.Header
		>
		<div class="settings-row">
			<span
				><strong>Appearance</strong><small
					>Choose your preferred theme</small
				></span
			><Button variant="outline" onclick={() => (dark = !dark)}
				>{#if dark}<Sun />Light{:else}<Moon />Dark{/if}</Button
			>
		</div>
		<div class="settings-row">
			<span
				><strong>Codex connection</strong><small
					>{app.connected
			...
			><Button
				variant="outline"
				disabled={!isTauri() || app.connecting || app.busy}
				onclick={() => app.connect()}
			...
			<span
				><strong>Account</strong><small
					>{app.account?.email ||
						...
						"Not signed in"}</small
				></span
			>{#if !app.account}<Button
					disabled={!app.connected}
					onclick={() => app.login()}>Sign in</Button
				>{:else}<span class="account-plan"
					>{app.account.planType || app.account.type}</span
				>{/if}
		</div>
		<p class="settings-note">
			ReCodex uses your installed Codex CLI and its sign-in. If Codex is
			not found, set <code>RECODEX_CODEX_PATH</code> to the executable path
			and restart the app.
		</p>
		{#if app.logs.length}<details class="diagnostics">
	...
	><Dialog.Content
		><Dialog.Header
			><Dialog.Title>Rename project</Dialog.Title><Dialog.Description
				>Change the name shown in your sidebar.</Dialog.Description
			></Dialog.Header
		>
		<form
